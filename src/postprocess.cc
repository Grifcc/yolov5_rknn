...
#define LABEL_NALE_TXT_PATH "./model/Visdrone_classes.txt"
...
int loadLabelName(const char *locationFilename, char *label[])
{
  ...
  printf("loadLabelName %s fisnished\n", locationFilename);
  return 0;
}
                 ...
                 std::vector<float> &qnt_scales, detect_result_group_t *group)
{
  ...
  std::stringstream ss;
  for (size_t i = 0; i < objProbs.size(); i++)
  {
    ss << filterBoxes[i * 4 + 0] << " "
       << filterBoxes[i * 4 + 1] << " "
       << filterBoxes[i * 4 + 2] << " "
       << filterBoxes[i * 4 + 3] << " "
       << objProbs[i] << " "
  ...
  for (int i = 0; i < validCount; ++i)
  {
    ...
    group->results[last_count].box.y2 = (int)y2;
